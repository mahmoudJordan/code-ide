import editorManager from './editorTabPanel'; // Assuming editorManager is the class instance exported from editorTabPanel
...
function buildFolderNavigator(projectName, nodes, parentElement) {
  ...
  nodes.forEach(node => {
    ...
    element.prepend(getFileIconEl(node));
...
async function navigate(projectName, node) {
  ...
  let fileContent = node.content ?? await getFileContent(projectName, node.filePath).then(response => response.data);
